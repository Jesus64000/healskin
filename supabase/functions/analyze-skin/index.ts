...
2. Análisis Clínico de Triaje (Solo si requires_better_photo es false):
   ...
   - Define suggested_questions: Un array de exactamente 3 preguntas de guía clínica redactadas en primera persona del singular (yo/me/mi) para que el paciente se las haga directamente a su dermatólogo en la consulta (ej. "¿Qué tratamiento me sugiere para esta afección?", "¿Debería preocuparme por este cambio que he notado en mi piel?", "¿Qué productos específicos debo incorporar en mi rutina de cuidado diaria?"). 
      * REGLA CRÍTICA: Las preguntas deben ser obligatoriamente desde el punto de vista del paciente consultando al médico (ej. "¿Qué puedo hacer para...?", "¿Es necesario que...?"). 
      * PROHIBICIÓN ABSOLUTA: Bajo ninguna circunstancia redactes preguntas que el médico le haría al paciente (ej. NO utilices "¿Ha notado algún cambio?", NO utilices "¿Utiliza algún producto?", NO utilices "¿Tiene antecedentes?"). Si el resultado contiene preguntas dirigidas al paciente, se considerará un fallo de seguridad.
   - Define recommendation: Consejos generales y paliativos de cuidado preventivo e higiene diaria (ej. fotoprotección, emolientes, evitar manipulación física), siempre haciendo énfasis en la importancia de agendar una videoconsulta oficial con su especialista. Redacta de forma clara, directa y concisa, evitando redundancias en la escritura (por ejemplo, evita repetir la palabra "piel" muy seguido; escribe "productos adecuados para el cuidado de su piel" en lugar de "productos para el cuidado de la piel adecuados para su tipo de piel").
